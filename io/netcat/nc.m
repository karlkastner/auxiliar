...
%   $Revision: 1.1.6.11.2.1 $  $Date: 2012/07/18 20:04:59 $
    ...
    methods
        ...
        function data = read(this,location, start, count, stride)
            ...
            if(strcmpi(vinfo.Datatype,'char') || ...
                    strcmpi(vinfo.Datatype,'uint64') ||...
                    strcmpi(vinfo.Datatype,'int64'));
...
%                    data       = data + double(add_offset);
        ...
        function write(this, fullVarName, varData, start, stride)
            ...
            if(~isempty(varinfo.Attributes) && ... % some attributes are present
                    ...
                    isa(varData,'double'))                  % data is double
                                
               
                %Subtract offset if present
                try
                    add_offset = netcdf.getAtt(gid,varid,'add_offset');
                    varData    = varData - double(add_offset);
                catch ALL %#ok<NASGU>
                    %do nothing
                end
                
                %Divide by  scale factor if present
                try
                    scale_factor = netcdf.getAtt(gid,varid,'scale_factor');
                    varData      = varData ./ double(scale_factor);
                catch ALL %#ok<NASGU>
                    %do nothing
                end
                ...
                % The low-level automatically casts the data.
    ...
    methods (Access=protected)
        ...
        function openToRead(this)
            if(strncmp(this.Filename,'http:',5))
                % OPeNDAP link
            else
        ...
        function writeVariableSchema(this, schemaStructs, fullGroupName)
            ...
            for infoInd = 1:length(schemaStructs)
                ...
                if(isempty(schema.Dimensions))
                    ...
                    % and the lengths match. Will error if lengths dont
                    % match.
    ...
    methods(Access=protected)
        ...
        function dispncVars(this, vinfo, indentSpace)
            ...
            for vInd = 1:length(vinfo)
                ...
                %dislpay the attributes for this variable if
            ...
            for aInd = 1:length(ainfo)
                if(isnumeric(ainfo(aInd).Value))
                    if(numel(ainfo(aInd).Value)>1)
                        ...
                        attDesc{aInd} = ...
                            ['[' num2str(ainfo(aInd).Value,'%4.2e ') ']'];
                    else
                        attDesc{aInd} = num2str(ainfo(aInd).Value, 3);
                    end
